...
class DiscreteMeanQFunction(DiscreteQFunction, nn.Module):  # type: ignore
    ...
    def compute_error(
        ...
        # TODO LiJiang check the actions tensor type.
        device = observations.device
        actions = actions.type(torch.LongTensor).view(-1).to(device)
        one_hot = F.one_hot(actions, num_classes=self.action_size)
