...
from .torch.iql_impl import IQLImpl, IQLDiscreteImpl
from .torch.sql_impl import SQLDiscreteImpl
from ..argument_utility import (
    EncoderArg,
    QFuncArg,
    RewardScalerArg,
...
class DiscreteSQL(AlgoBase):
    ...
    Args:
        ...
        reward_scaler (d3rlpy.preprocessing.RewardScaler or str):
            reward preprocessor. The available options are
            ``['clip', 'min_max', 'standard']``.
        impl (d3rlpy.algos.torch.iql_impl.IQLDiscreteImpl): algorithm implementation.
    ...
    _max_weight: float
    _use_gpu: Optional[Device]
    _impl: Optional[IQLDiscreteImpl]

    def __init__(
        self,
        *,
        actor_learning_rate: float = 3e-4,
        critic_learning_rate: float = 3e-4, # v and q
        actor_optim_factory: OptimizerFactory = AdamFactory(),
        critic_optim_factory: OptimizerFactory = AdamFactory(),
        ...
        action_scaler: ActionScalerArg = None,
        reward_scaler: RewardScalerArg = None,
        impl: Optional[IQLDiscreteImpl] = None,
        **kwargs: Any,
    ):
        super().__init__(
            batch_size=batch_size,
