...
class SQLDiscreteImpl(DDPGBaseImpl):
    _policy: Optional[NonSquashedNormalPolicy]
    # _expectile: float
    # _weight_temp: float
    _alpha:float
    ...
    def compute_actor_loss(self, batch: TorchMiniBatch) -> torch.Tensor:
        ...
        actor_loss = -((diff > 0).float() * log_probs).mean()
        return actor_loss
    ...
    def compute_value_loss(self, batch: TorchMiniBatch) -> torch.Tensor:
        ...
        # embed()
        v_t = self._value_func(batch.observations)
        diff = 0.5 + (q_t.detach() - v_t) / self._alpha
        weight = (diff > 0.0).float().detach()
        return (weight * (diff**2) + v_t / self._alpha).mean()
